...
import {
  ...
  doc,
  Timestamp,
} from "firebase/firestore"; // Importa los métodos necesarios de Firestore
import {
  createUserProfile,
  getUserProfileById,
  updateUserProfile,
...
export async function updateUserPhoto(photo) {
  try {
    ...
    // TODO: Manejar el error.
    console.error(
      "[auth.js updateUserPhoto] Error al actualizar la foto de perfil."
    );
    throw error;
  }
}
  ...
  return onSnapshot(
    ...
    (snapshot) => {
      ...
      if (publicaciones.length === 0) {
        console.log(
          "No se encontraron publicaciones para el usuario con email:",
          userEmail
        );
      }
